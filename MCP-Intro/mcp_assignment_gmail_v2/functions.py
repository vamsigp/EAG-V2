...
# Load env from .env so server has access to credentials when launched via stdio
load_dotenv()
...
async def send_email(to: str, subject: str, body: str) -> dict:
    """Send an email using Gmail SMTP. If 'to' is empty, uses EMAIL_TO from env."""
    try:
        ...
        recipient = to or (default_to or "")
        if not recipient:
            return {
                "content": [
                    TextContent(type="text", text="No recipient email provided.")
                ]
            }
        ...
        return {
            "content": [
                TextContent(type="text", text=f"Email sent to {recipient}")
            ]
        }
    except Exception as e:
        return {
