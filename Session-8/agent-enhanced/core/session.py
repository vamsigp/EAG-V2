...
try:
    ...
    SSE_AVAILABLE = False
    print("Warning: SSE client not available. Install with: pip install mcp[cli]")
...
class MultiMCP:
            ...
            try:
                ...
                if transport == "sse":
                    if not SSE_AVAILABLE:
                        print(f"❌ SSE not available for {config['script']}. Install mcp[cli]")
                        continue
                    
                    # SSE transport
                    sse_url = config.get("sse_url") or f"http://localhost:{config.get('port', 8001)}/sse"
                    print(f"→ Connecting to SSE server: {sse_url}")
                    
                    try:
                        async with sse_client(sse_url) as (read_stream, write_stream):
                            async with ClientSession(read_stream, write_stream) as session:
                                print("[agent] SSE session created, initializing...")
                                await session.initialize()
                                print("[agent] SSE MCP session initialized")
                        ...
                        print(f"❌ SSE connection error: {se}")
                ...
                print(f"❌ Error initializing MCP server {config.get('script', 'unknown')}: {e}")
    ...
    async def call_tool(self, tool_name: str, arguments: dict) -> Any:
        ...
        if transport == "sse":
            # SSE transport
            if not SSE_AVAILABLE:
                raise RuntimeError("SSE transport not available")
